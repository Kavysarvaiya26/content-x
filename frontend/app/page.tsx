...
export default function TransformAIDashboard() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#090d16] text-slate-100">
      {/* Header Bar */}
      <header className="sticky top-0 z-50 border-b border-slate-800 bg-[#0b1329]/80 backdrop-blur-md px-6 py-4 flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 rounded-xl glow-button flex items-center justify-center text-white shadow-lg">
            <Sparkles className="w-5 h-5 animate-pulse" />
          </div>
          <div>
            <h1 className="text-xl font-bold tracking-tight bg-gradient-to-r from-indigo-400 via-purple-300 to-emerald-400 bg-clip-text text-transparent">
              TransformAI
            </h1>
        ...
        <div className="flex items-center space-x-4">
          ...
          {jobId && (
            <div className="text-xs font-mono bg-slate-800/80 border border-slate-700 px-3 py-1.5 rounded-lg text-slate-300">
              JOB: {jobId.slice(0, 8)}...
            </div>
          )}
        </div>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-6 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <section className="lg:col-span-6 glass-panel rounded-2xl p-6 flex flex-col justify-between border border-slate-800">
            <div>
              ...
              {/* Ingestion Type Tabs */}
              <div className="flex border-b border-slate-800 mb-4 space-x-4">
                <button
              ...
              {ingestTab === "text" && (
                <textarea
                  ...
                  className="w-full bg-slate-900/90 border border-slate-800 rounded-xl p-3 text-sm text-slate-200 focus:outline-none focus:border-indigo-500 font-mono resize-none"
                />
              )}

              {ingestTab === "file" && (
                <div className="border-2 border-dashed border-slate-800 rounded-xl p-8 text-center hover:border-indigo-500/50 transition-colors">
                  ...
                  <label
                    ...
                    className="cursor-pointer text-sm text-indigo-400 font-medium hover:underline"
                  >
              ...
              {ingestTab === "url" && (
                <div className="space-y-3">
                  <input
                    ...
                    className="w-full bg-slate-900/90 border border-slate-800 rounded-xl p-3 text-sm text-slate-200 focus:outline-none focus:border-indigo-500"
                  />
            ...
            <div className="mt-6 space-y-4">
              <button
                ...
                disabled={uploading}
                className="w-full py-2.5 px-4 bg-slate-800 hover:bg-slate-700 text-slate-200 font-medium rounded-xl border border-slate-700 transition-colors flex items-center justify-center space-x-2 text-sm disabled:opacity-50"
              >
                {uploading ? (
                  <>
                  ...
                  <>
                    ...
                    <span>Ingest & Extract Source</span>
                  </>
                )}
              </button>

              {source && (
                <div className="bg-emerald-950/20 border border-emerald-500/30 rounded-xl p-3 flex items-center justify-between text-xs">
                  ...
                  <span className="font-mono text-emerald-400 text-[10px] bg-emerald-900/40 px-2 py-0.5 rounded">
          ...
          <section className="lg:col-span-6 glass-panel rounded-2xl p-6 flex flex-col justify-between border border-slate-800">
                ...
                {DELIVERABLE_TYPES.map((item) => {
                  ...
                  return (
                    <div
                      ...
                      className={`cursor-pointer rounded-xl p-3 border transition-all flex items-start space-x-3 ${
                        isSelected
                          ? "bg-indigo-950/30 border-indigo-500/60 shadow-md shadow-indigo-500/10"
                          : "glass-card border-slate-800 opacity-60 hover:opacity-100"
                      }`}
                    >
                      <input
                        type="checkbox"
                        checked={isSelected}
                        onChange={() => {}}
                        className="mt-1 rounded text-indigo-600 focus:ring-indigo-500 bg-slate-900 border-slate-700"
                      />
                      <div>
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-900/90 border border-slate-800 rounded-lg p-2 text-slate-200 focus:outline-none focus:border-indigo-500"
                  />
                </div>
                <div>
            ...
            <div className="mt-6">
              <button
                ...
                className="w-full py-3.5 px-6 glow-button text-white font-semibold rounded-xl transition-all flex items-center justify-center space-x-3 text-base shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {executing ? (
                  <>
                    <RefreshCw className="w-5 h-5 animate-spin" />
                    <span>LangGraph Pipeline Running...</span>
                  </>
                ) : (
                  <>
        ...
        <section className="glass-panel rounded-2xl p-6 border border-slate-800">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-slate-200 flex items-center space-x-2">
              ...
              <span>LangGraph Execution Pipeline Nodes</span>
            </h3>
            {currentNode && (
              <span className="text-xs font-mono bg-indigo-950/60 border border-indigo-500/40 text-indigo-300 px-2.5 py-1 rounded-full">
                Node: {currentNode}
              </span>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
            {PIPELINE_NODES.map((node, index) => {
              ...
              const isPast =
                jobStatus === "completed" ||
                (currentNode &&
                  PIPELINE_NODES.findIndex((n) => n.id === currentNode) > index);
              ...
              return (
                <div
                  ...
                  className={`p-3 rounded-xl border text-center transition-all flex flex-col items-center justify-center space-y-1.5 ${
                    isActive
                      ? "node-active border-indigo-500 bg-indigo-950/40 text-indigo-200 shadow-lg shadow-indigo-500/20"
                      : isPast
                ...
                >
                  ...
                  {isActive && <RefreshCw className="w-3 h-3 animate-spin text-indigo-400" />}
          ...
          <section className="glass-panel rounded-2xl p-6 border border-slate-800 space-y-6">
            ...
            <div className="flex flex-wrap items-center justify-between border-b border-slate-800 pb-4 gap-4">
              <div className="flex flex-wrap gap-2">
                {jobData.outputs.map((output: any) => {
                  const isActive = activeOutputTab === output.output_type;
                  const isSuccess = output.status === "succeeded" || output.status === "repaired";
                  return (
                    <button
                      key={output.id}
                      onClick={() => {
                        setActiveOutputTab(output.output_type);
                        setEditText(output.content_markdown || "");
                      }}
              ...
              <div className="flex items-center space-x-2">
                ...
                <button
                  onClick={() => {
                    setViewMode("edit");
                    setEditText(activeOutput?.content_markdown || "");
                  }}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium ${
                    viewMode === "edit"
                      ? "bg-slate-800 text-slate-100"
            ...
            {activeOutput && (
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between text-xs">
                  <div className="flex items-center space-x-2">
                    <span className="text-slate-400">Fact Keys Used:</span>
                    {activeOutput.fact_keys_used && activeOutput.fact_keys_used.length > 0 ? (
                      activeOutput.fact_keys_used.map((key: string) => (
                        <span
                          key={key}
                          className="bg-indigo-950 border border-indigo-500/40 text-indigo-300 px-2 py-0.5 rounded-md font-mono"
                        >
                          {key}
                        </span>
                      ))
                    ) : (
                      <span className="text-slate-500 italic">None claimed</span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <button
                      onClick={() => handleCopyMarkdown(activeOutput.content_markdown)}
                      className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg text-xs flex items-center space-x-1.5 transition-colors"
                    >
                    ...
                    {activeOutput.output_type === "presentation" && (
                      <a
                        ...
                        download
                        className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs flex items-center space-x-1.5 font-medium transition-colors shadow-md"
                      >
                ...
                {/* View Content Panels */}
                {viewMode === "rendered" && (
                  <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-6 text-slate-200 text-sm leading-relaxed font-sans whitespace-pre-wrap max-h-[500px] overflow-y-auto">
                    {activeOutput.content_markdown || "No markdown rendered."}
                  </div>
                )}

                {viewMode === "raw" && (
                  <pre className="bg-slate-950 border border-slate-800 rounded-xl p-4 text-xs font-mono text-indigo-300 max-h-[500px] overflow-y-auto">
                    {JSON.stringify(activeOutput.content_json, null, 2)}
                  </pre>
                )}
                ...
                {viewMode === "edit" && (
                  <div className="space-y-3">
                    <textarea
                      ...
                      rows={12}
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-sm font-mono text-slate-200 focus:outline-none focus:border-indigo-500"
                    />
                    <div className="flex justify-end space-x-3">
                      ...
                      <button
                        ...
                        disabled={savingEdit}
                        className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-semibold flex items-center space-x-2"
                      >
                        {savingEdit && <RefreshCw className="w-3.5 h-3.5 animate-spin" />}
                        <span>Save Inline Edit</span>
                      </button>
                    </div>
                  </div>
                )}
                ...
                {jobData.validations && (
                  <div className="border-t border-slate-800 pt-6">
                    <h4 className="text-xs font-semibold text-slate-300 uppercase tracking-wider mb-3 flex items-center space-x-2">
                      ...
                      <span>Validation Health Audits</span>
                    </h4>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {["consistency", "grounding", "quality"].map((checkType) => {
                        const reports = jobData.validations.filter(
                          (v: any) =>
                            v.check_type === checkType &&
                            (!v.output_id ||
                              v.output_id === activeOutput.id ||
                              v.details?.output_type === activeOutput.output_type)
                        );

                        const allPassed =
                          reports.length > 0 && reports.every((r: any) => r.passed);
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center justify-between mb-2">
                              ...
                              {allPassed ? (
                                <span className="bg-emerald-950 border border-emerald-500/40 text-emerald-400 text-[10px] px-2 py-0.5 rounded-full font-semibold">
                                  PASSED
                                </span>
                              ) : (
                                <span className="bg-amber-950 border border-amber-500/40 text-amber-400 text-[10px] px-2 py-0.5 rounded-full font-semibold">
                                  WARNING
                                </span>
                              )}
                            </div>
                            <p className="text-[11px] text-slate-400">
                              {checkType === "consistency" && "Fact conflict verification"}
                              {checkType === "grounding" && "Canonical source traceability"}
                              {checkType === "quality" && "Payload structure validation"}
                            </p>
                          </div>
                        );
                      })}
            ...
            <div className="flex justify-end pt-4">
              <button
                ...
                className="px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 rounded-xl text-xs font-medium flex items-center space-x-2 transition-colors"
              >
                <RefreshCw className="w-3.5 h-3.5 text-indigo-400" />
                <span>Regenerate All Deliverables</span>
              </button>
            </div>
          </section>
        )}
